...
class MemoryAccess(snt.RNNCore):
    ...
    *   `addressing.FreenessAllocator` for keeping track of memory usage, where
    ...
    def __init__(self,
        ...
        self._write_content_weights_mod = _addressing.CosineWeights(
            num_writes, word_size, name='write_content_weights')
        self._read_content_weights_mod = _addressing.CosineWeights(
            num_reads, word_size, name='read_content_weights')

        self._linkage = _addressing.TemporalLinkage(memory_size, num_writes)
        self._freeness = _addressing.Freeness(memory_size)
